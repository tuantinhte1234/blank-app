...
st.markdown("<div style='background-color: #ffffff; padding: 10px; border-radius: 5px; color: black;'>", unsafe_allow_html=True)
st.write("**Session Details**")
session_info = df[df["Content"] == selected_session].iloc[0]
st.write(f"Time: {session_info['Time']}")
st.markdown(f"Speaker: <span class='speaker' style='color: black;'>{session_info['Speaker']}</span>", unsafe_allow_html=True)
